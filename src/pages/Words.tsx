...
const Words = () => {
  ...
  const allVocabulary = [
    ...
    // Intermediate words for Basic users
    {
      word: "Achievement",
  ...
  // Filter vocabulary based on user's plan access level
  ...
  const vocabulary = allVocabulary.filter(word => accessibleLevels.includes(word.level));
  ...
  const canAccessWord = (level: string) => {
    const accessibleLevels = getAccessibleLevels(userPlan);
    return accessibleLevels.includes(level);
  };

  const handleMarkAsLearned = async () => {
    // Check if user can access this word level
    if (!canAccessWord(currentWord.level)) {
      toast({
        title: "Nivel no disponible",
        description: `Necesitas un plan superior para acceder a palabras de nivel ${currentWord.level}.`,
        variant: "destructive"
      });
      return;
    }
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-4">
                  ...
                  <Button
                    ...
                    disabled={
                      learnedWords.has(currentWordIndex) || 
                      !getDailyLimits().canLearnMore ||
                      !canAccessWord(currentWord.level)
                    }
                  >
